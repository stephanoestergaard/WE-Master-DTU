...
CONTAINS
!-------------------------------------------------------------------------------------------------------------------------------
    SUBROUTINE StateMachine(CntrPar, LocalVar, DebugVar)
    ...
    !       VS_State = 6, Tip-Speed-Ratio tracking PI controller
        ...
        DebugVar%GenArTq   = LocalVar%GenArTq
...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       SS_Mode = 1, Implement setpoint smoothing
        ...
        IF ( CntrPar%SS_Mode == 1) THEN
            ...
            DelOmega = ((LocalVar%PC_PitComT - CntrPar%PC_MinPit)/0.524) * CntrPar%SS_VSGain - (CntrPar%VS_MaxOMTq - LocalVar%VS_LastGenTrq)/CntrPar%VS_MaxOMTq * CntrPar%SS_PCGain ! Normalize to 30 degrees for now
