...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       Sine excitation on pitch    
        ...
        ! FloatingFeedback
        IF ((CntrPar%Fl_Mode == 1) .OR. (CntrPar%Fl_Mode == 3)) THEN
...
!-------------------------------------------------------------------------------------------------------------------------------
    SUBROUTINE IPC(CntrPar, LocalVar, objInst, DebugVar)
        ...
        REAL(8)                  :: Fl_IPC_Gain                             ! Floating IPC gain
        ...
        ! FloatingFeedback IPC
        IF (CntrPar%Fl_Mode >= 2) THEN
            Fl_IPC_Gain = FloatingFeedbackIPC(LocalVar, CntrPar, objInst)
            Fl_IPC_Gain = saturate(Fl_IPC_Gain, -CntrPar%IPC_IntSat, CntrPar%IPC_IntSat)
        ELSE 
            Fl_IPC_Gain = 0.0
        ENDIF
        ...
        IF ((CntrPar%IPC_ControlMode >= 1) .AND. (CntrPar%Y_ControlMode /= 2)) THEN
            ...
            ! Add floating feedback contribution
        ...
        DebugVar%IntAxisTilt_1P = IntAxisTilt_1P
        DebugVar%IntAxisYaw_1P = IntAxisYaw_1P
        DebugVar%Fl_IPC_Gain = Fl_IPC_Gain
        ...
        CALL ColemanTransformInverse(IntAxisTilt_1P + Fl_IPC_Gain, IntAxisYawIPC_1P, LocalVar%Azimuth, NP_1, CntrPar%IPC_aziOffset(1), PitComIPC_1P)
...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       Fl_Mode = 2, Proportional feedback of nacelle velocity on IPC
    !       Fl_Mode = 3, Proportional feedback of nacelle velocity on both
        ...
        FloatingFeedback = (0.0 - NacIMU_FA_vel) * CntrPar%Fl_Kp(1) !* LocalVar%PC_KP/maxval(CntrPar%PC_GS_KP)

    END FUNCTION FloatingFeedback
!-------------------------------------------------------------------------------------------------------------------------------
    REAL FUNCTION FloatingFeedbackIPC(LocalVar, CntrPar, objInst) 
    ! FloatingFeedback defines a minimum blade pitch angle based on a lookup table provided by DISON.IN
    !       Fl_Mode = 0, No feedback
    !       Fl_Mode = 1, Proportional feedback of nacelle velocity on collective pitch
    !       Fl_Mode = 2, Proportional feedback of nacelle velocity on IPC
    !       Fl_Mode = 3, Proportional feedback of nacelle velocity on both
        USE ROSCO_Types, ONLY : LocalVariables, ControlParameters, ObjectInstances
        IMPLICIT NONE
        ...
        ! Calculate floating contribution to pitch command
        NacIMU_FA_vel = PIController(LocalVar%NacIMU_FA_AccF, 0.0, 1.0, -100.0 , 100.0 ,LocalVar%DT, 0.0, .FALSE., objInst%instPI) ! NJA: should never reach saturation limits....
        FloatingFeedbackIPC = (0.0 - NacIMU_FA_vel) * CntrPar%Fl_Kp(2) !* LocalVar%PC_KP/maxval(CntrPar%PC_GS_KP)

    END FUNCTION FloatingFeedbackIPC
!-------------------------------------------------------------------------------------------------------------------------------
